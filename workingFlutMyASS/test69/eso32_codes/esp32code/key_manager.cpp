    ...
    bool generateAndStoreMasterKey() {
        ...
        Serial.printf("[KEY_MGR] Storing master key in namespace '%s' with key '%s'...\n", NVS_NAMESPACE, MASTER_KEY_NVS_KEY);
        ...
        Serial.println("[KEY_MGR] Master key stored in NVS buffer...");
        ...
        Serial.println("[KEY_MGR] Initialization flag set...");
        
        // Commit changes to flash - THIS IS CRITICAL!
        Serial.println("[KEY_MGR] Committing to NVS flash...");
        ret = nvs_commit(nvs_handle_storage);
        if (ret != ESP_OK) {
            Serial.printf("[KEY_MGR] ❌ CRITICAL: Failed to commit master key to NVS: %s\n", esp_err_to_name(ret));
            secureZero(masterKey, sizeof(masterKey));
            return false;
        }
        Serial.println("[KEY_MGR] ✅ NVS commit successful - data should be persistent now");
        
        // Force NVS handle refresh to ensure immediate read capability
        nvs_close(nvs_handle_storage);
        delay(50); // Longer delay for NVS state stabilization
        
        ret = nvs_open(NVS_NAMESPACE, NVS_READWRITE, &nvs_handle_storage);
        if (ret != ESP_OK) {
            Serial.printf("[KEY_MGR] Failed to reopen NVS handle after storing key: %s\n", esp_err_to_name(ret));
            key_manager_initialized = false;
            secureZero(masterKey, sizeof(masterKey));
            return false;
        }
        
        // Verify the write AFTER handle refresh
        size_t required_size = MASTER_KEY_SIZE;
        uint8_t test_read[MASTER_KEY_SIZE];
        ret = nvs_get_blob(nvs_handle_storage, MASTER_KEY_NVS_KEY, test_read, &required_size);
        if (ret == ESP_OK && required_size == MASTER_KEY_SIZE) {
            Serial.println("[KEY_MGR] ✅ Post-refresh verification: Master key read back successfully");
            secureZero(test_read, sizeof(test_read));
        } else {
            Serial.printf("[KEY_MGR] ❌ Post-refresh verification FAILED: %s (size: %d)\n", esp_err_to_name(ret), required_size);
            Serial.println("[KEY_MGR] ⚠️ This indicates a serious NVS partition issue");
        }
    ...
    bool loadMasterKey(uint8_t* key) {
        ...
        Serial.printf("[KEY_MGR] Loading master key from namespace '%s' with key '%s'...\n", NVS_NAMESPACE, MASTER_KEY_NVS_KEY);
        ...
        if (ret != ESP_OK) {
            ...
            Serial.println("[KEY_MGR] NVS handle reopened, trying read again...");
    ...
    bool getMasterKey(uint8_t* key) {
        ...
        if (!masterKeyExists()) {
            ...
            // Add a longer delay to ensure NVS write completes and stabilizes
            delay(50);
